---
label: LOCAL
standing: user
authority: >-
  user ruling, 2026-09-01: "That's true for now. The door should be
  open to s3fs plus lambda or something in future, but that's a
  fourth-order concern for today."
---

# The substrate is this machine, and nothing forecloses another

The substrate is this machine: a local filesystem and a process table.
Remote storage and remote compute stay possible, and no shape decision
may foreclose them, but they are a fourth-order concern today.
